...
public class SocialPlayClient {
    ...
    private static final long DEFAULT_USER_ID = 1234567L;
    ...
    public static long createSettings() {
        try {
            SettingsCreateRequestBuilder settingsCreateRequestBuilder = new SettingsRequestBuilders().create();
            ...
            stringArray.add("12:10:00");
            DaySchedule daySchedule = new DaySchedule().setDay(Day.EVERYDAY).setSchedules(stringArray);
            dayScheduleArray.add(daySchedule);
            settings.setSchedules(dayScheduleArray);

            CreateIdRequest<Long, Settings> createRequest = settingsCreateRequestBuilder.input(settings).build();
            System.out.println("request: " + createRequest.toSecureString());
            ResponseFuture<IdResponse<Long>> responseFuture = restClient.sendRequest(createRequest);
            Response<IdResponse<Long>> response = responseFuture.getResponse();
            return response.getEntity().getId();
        } catch (RemoteInvocationException ex) {
            System.out.println("Encountered error doing create settings: " + ex.getMessage());
        }
